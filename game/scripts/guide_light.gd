...
func _process(delta: float) -> void:
	...
	if Global.player_view != Global.VIEW.FLAME and Global.player_view != Global.VIEW.FORCE and (light_level > 0 and new_light_range >= light_level) or (light_level < 0 and new_light_range < 0):
		#light_range = 0.1
